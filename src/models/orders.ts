...
export default class OrderSchema {
    ...
    async create(order: Order): Promise<Order> {
        try {
            ...
            throw new Error(`can not create the ordre (from models) ==> ${error}`);
        }
    }
    /* -------------------------------------------- delete an order -------------------------------------------- */
    async delete(id: number): Promise<Order> {
        try {
            ...
            throw new Error(`can not delete the order (from models) ==> ${error}`);
        }
    }

    /* -------------------------------------------- add products to order -------------------------------------------- */
    async addProducts(quantity:number, orderId:number, productId:number): Promise<Order_products> {
        try {
            const connection = await dbServer.connect();
            ...
            throw new Error(`can not add products to the order (from models) ==> ${error}`);
        }
    }
    ...
    async showProducts(orderId:number): Promise<Order_products[]>{
        try {
            ...
            throw new Error(`can not show products of specific order (from models) ==> ${error}`);
        }
    }
}
